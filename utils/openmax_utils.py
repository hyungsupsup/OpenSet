...
def compute_distance(query_channel, channel, mean_vec, distance_type='eucos'):
    ...
    query_channel = np.array(query_channel).flatten()
    print('flattened query_channel: ', query_channel.shape)
    print('shape: ', mean_vec.shape)
    #mean_vec = np.reshape(mean_vec, (5, 1))   # 여기서 에러
    mean_vec = mean_vec.flatten()
    print('flattened shape: ', mean_vec.shape)
